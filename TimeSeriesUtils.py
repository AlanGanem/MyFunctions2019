...
def n_of_week_days(df, merge_axis = 1, prefix = 'n_'):
    assert df.index.dtype == '<M8[ns]'
    n_days = {}
    for date in tqdm.tqdm(df.index.unique()):
        n_days[date] = {}
        for row in range(df.shape[0]):
            for day in range(7):            
                n_days[date][day] = len([i for i in calendar.monthcalendar(date.year, date.month) if i[day] != 0])
            
    day_name_map = {0:'Monday',1:'Tuesday',2:'Wednesday',3:'Thursday',4:'Friday',5:'Saturday',6:'Sunday'}
    day_name_map_inv = {value:key for key,value in day_name_map.items()}
    col_names = [i for i in list(day_name_map.values())]
    for key in n_days.keys():
        for col in col_names:
            df.loc[key,prefix+col] = n_days[key][day_name_map_inv[col]]
    
    return df, [prefix+i for i in list(day_name_map.values())]
