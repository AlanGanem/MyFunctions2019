...
###################################################################
def kernel_density_apply(segmentation_policy, dataframe):
    import pandas as pd
    import time
    s = time.time()
    data=dataframe.copy()    
    ...
    for i in unique_categories:
        ...
        for j in range_:                
            if j == min(range_):
                indexes = data[data.category_id == int(unchanged_cat_str)].index[data[data.category_id == int(unchanged_cat_str)].price_min.between(0,max(segmentation_policy[unchanged_cat_str][j]),inclusive=True)]
            elif j == max(range_):
                indexes = data[data.category_id == int(unchanged_cat_str)].index[data[data.category_id == int(unchanged_cat_str)].price_min.gt(min(segmentation_policy[unchanged_cat_str][j-1]))]
            else:
                indexes = data[data.category_id == int(unchanged_cat_str)].index[data[data.category_id == int(unchanged_cat_str)].price_min.between(min(segmentation_policy[unchanged_cat_str][j]),max(segmentation_policy[unchanged_cat_str][j]))]
...
################################################################
def kernel_density_clustering(dataframe, min_items = 2, min_var_coef = 0.3,column='category_id'):
    import time
    ...
    segmentation_policy={}
    for i in unique_categories:
        x = eval('np.sort(data[data.%s == i].price_min.values)' % column)        
        ...
        if (h > 0)&(len(x)>2):
            if var_coef_x >= min_var_coef:
                ...
                while j in range(len(x_split)):                
                    indexes = eval('data[data.%s == i].index[data[data.%s == i].price_min.isin(x_split[j])]' % (column,column))
                    new_category = str(unchanged_cat_str + '_' + str(j))
            ...
            if var_coef_x <= min_var_coef:
                ...
                unchanged_cat_str = str(int(i))
                index_max = eval('data[data.%s == i].index[data[data.%s == i].price_min == max(data[data.%s == i].price_min)]' % (column,column,column))
                index_min = eval('data[data.%s == i].index[data[data.%s == i].price_min == min(data[data.%s == i].price_min)]' % (column,column,column))
